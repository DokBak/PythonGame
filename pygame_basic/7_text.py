...
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) # 가로의 절반 크기에 해당하는 곳에 위치 (가로) # 가로의 절반 크기에서 적 절반 크기만큼 빼주어야 캐릭터가 중앙으로 간다.
enemy_y_pos = (screen_height / 2) - (enemy_height / 2) # 화면 세로크기 가# 화면장 아래에 해당하는 곳에 위치 (세로) # 전체 높이에서 적 높이만큼 빼주어야 캐릭터의 높이 위치가 계산된다.

# 폰트 정의
game_font = pygame.font.Font(None,40) # 폰트 객체 생성 (폰트, 크기)

# 총 시간 (이번 프로젝트는 제한시간을 두는 방식으로 시간이 감소하는 형식으로 사용하기 때문)
total_time = 10

# 시작 시간 
start_ticks = pygame.time.get_ticks() # 현재 tick을 받아 시작시간 변수에 넣어둔다.
